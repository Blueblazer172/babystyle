...
class NewActionTest extends AbstractBackendController
{
    public function setUp()
    {
        ...
        $this->uri = 'backend/search/stopword/new';

        parent::setUp();
    }

    /**
     * @covers Mirasvit\Search\Controller\Adminhtml\Stopword\NewAction::execute
     */
    public function testExecute()
    {
        $this->dispatch('backend/search/stopword/new');

        $this->assertFalse($this->getResponse()->isRedirect(), 'Wrong redirect at edit page.');
        $this->assertSessionMessages($this->isEmpty(), MessageInterface::TYPE_ERROR);

        $this->assertContains(
            '<h1 class="page-title">New Stopword</h1>',

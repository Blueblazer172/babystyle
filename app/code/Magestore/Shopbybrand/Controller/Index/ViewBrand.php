...
class ViewBrand extends \Magento\Framework\App\Action\Action
{
    ...
    public function execute()
    {
        ...
        // update breadcrumbs
			...
			$url  = \Magento\Framework\App\ObjectManager::getInstance()->create('Magestore\Shopbybrand\Model\SystemConfig')->getFrontendUrlPath();
            if ($breadcrumbsBlock) {
                $breadcrumbsBlock->addCrumb(
                    'brands',
                    ['label' => __('Brands'), 'link' =>$this->_storeManager->getStore()->getUrl($url) , 'readonly' => true]
                );
                $breadcrumbsBlock->addCrumb('brandview', ['label' => $brand->getBrandName()]);
            }
			
        $resultPage->getConfig()->getTitle()->set( !empty($brand->getTitle()) ? $brand->getTitle() : $brand->getBrandName() );
		$resultPage->getConfig()->setKeywords( !empty($brand->getMetaKeywords()) ? $brand->getMetaKeywords() : $brand->getBrandName());
		$resultPage->getConfig()->setDescription( !empty($brand->getMetaDescription()) ? $brand->getMetaDescription() : $brand->getBrandName());

...
define([
    ...
    MageplazaOscAddress.prototype = {
        ...
        setBillingAddress: function (config) {
            ...
            this.billingAddress.billingPostCode = $(this.billingAddress.countryRegionElements.postcode);
        },
        setShippingAddress: function (config) {
            ...
            this.shippingAddress.shippingPostCode = $(this.billingAddress.countryRegionElements.postcode);
        ...
        initObserver: function () {
            ...
            var billingPostCode = this.billingAddress.billingPostCode;
            ...
            if (billingPostCode) {
                billingPostCode.observe('change', function () {
                    this.postcodeChanged(this.billingAddress);
                }.bind(this))
            }
            ...
            var shippingPostCode = this.shippingAddress.shippingPostCode;
            ...
            if (shippingPostCode) {
                shippingPostCode.observe('change', function () {
                    this.postcodeChanged(this.shippingAddress);
                }.bind(this))
            }
        ...
        postcodeChanged: function (targetAddress) {
            var postcodeElm = $(targetAddress.countryRegionElements.postcode)
            var postCode = postcodeElm.value;
            var countryId = $(targetAddress.countryRegionElements.countrySelect).value;
            postcodeElm.next('.message.warning') && postcodeElm.next('.message.warning').remove();
            var validationResult = this.validatePostCode(postCode,countryId);
            if (typeof validationResult == "string") {
                validationResult = "<div class=\"message warning\" generated=\"true\"><span>"
                + validationResult
                + "</span></div>";
                $(targetAddress.countryRegionElements.postcode).up().insert(validationResult);
            }
        },
        validatePostCode: function(postCode, countryId) {
            var patterns = window.checkoutConfig.postCodes[countryId];
            var validatedPostCodeExample = [];
            if (postCode.length > 0 && Object.keys(patterns).length > 0) {
                ...
                warnMessage = 'Provided Zip/Postal Code seems to be invalid.';
                if (validatedPostCodeExample.length) {
                    warnMessage += ' Example: ' + validatedPostCodeExample.join('; ') + '. ';
                }
                warnMessage += 'If you believe it is the right one you can ignore this notice.';
                return warnMessage;
            }
            return true;
        },
